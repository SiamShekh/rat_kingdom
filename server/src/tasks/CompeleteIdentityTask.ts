...
import { Collaboration_Task_Model } from "../collaboration task/collaboration_task.model";


const CompeleteIdentityTask = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    try {
        ...
        const task = await Collaboration_Task_Model.findById(taskId).session(session);
        console.log(task);
        ...
        await TrackingModel.create([{ category: task?.category || 'any', type: 'identity_task', point: task?.point, taskId: task?._id, userId: user?._id }], { session });
...
export default CompeleteIdentityTask;